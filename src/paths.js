...
// GITUPDATER_CONFIG already overrides the config file. The test suite uses them so it
// never touches a real install, and they give a portable-everything setup an escape hatch.
const configDir = () => process.env.GITUPDATER_CONFIG_DIR || path.join(configRoot(), APP);
const dataDir = () => process.env.GITUPDATER_DATA_DIR || path.join(dataRoot(), APP);

module.exports = { configDir, dataDir, configRoot, dataRoot, APP };
